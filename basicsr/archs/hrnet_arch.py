...
class Conv(nn.Module):
	def __init__(
			self, num_in_ch, num_out_ch, kernel_size=3, stride=1, relued=True
        ):
		...
		self.conv_bn = nn.Sequential(
            ...
            nn.BatchNorm2d(num_out_ch, momentum=BN_MOMENTUM)
        )
		self.relu = nn.ReLU()
		self.relued = relued
...
class BasicBlock(nn.Module):
	def __init__(self, num_in_ch, num_out_ch):
		...
		self.conv = nn.Sequential(
            Conv(num_in_ch, num_out_ch),
            Conv(num_in_ch, num_out_ch, relued=False)
        )
		self.relu = nn.ReLU()
	def forward(self, x):
		identity = x
...
class Bottleneck(nn.Module):
	...
	def __init__(self, num_in_ch, num_out_ch, downsampling=None):
		super(Bottleneck, self).__init__()
		self.conv = nn.Sequential(
            Conv(num_in_ch, num_out_ch, kernel_size=1),
            Conv(num_out_ch, num_out_ch),
            Conv(
                num_out_ch,
                num_out_ch * self.expansion,
                kernel_size=1,
                relued=False
            )
        )
...
class HRNet(nn.Module):
	def __init__(self, num_in_ch, num_mid_ch, num_out_ch, num_stage=4):
		...
		self.head = nn.Sequential(
            ...
            nn.Conv2d(num_mid_ch * (1 + 2 + 4 + 8), num_out_ch, 1)
		)
		self.first_layer = self._make_layer(64, 64, Bottleneck, 4)
		self.first_transition = self._make_transition_layer(256, num_mid_ch, 1)
		...
		for i in range(1, num_stage):
			self.hr_blocks.append(
				HRBlock(
					num_mid_ch,
					i + 1,
					True if i == num_stage - 1 else False,
					BasicBlock
                )
            )
	...
	def _make_layer(self, num_in_ch, ch, block, num):
		downsampling = None
		if num_in_ch != ch * block.expansion:
			downsampling = Conv(
				num_in_ch, ch * block.expansion, 1, relued=False)
		layers = []
		layers.append(block(num_in_ch, ch, downsampling))
		for i in range(1, num):
			layers.append(block(ch * block.expansion, ch))
		return nn.Sequential(*layers)

	def _make_transition_layer(self, num_in_ch, num_out_ch, stage):
		layers = nn.ModuleList()
		layers.append(Conv(num_in_ch, num_out_ch, 1))
		layers.append(Conv(num_in_ch, num_out_ch * 2, 3, 2))
		return layers
